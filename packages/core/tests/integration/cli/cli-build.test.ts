...
describe('Dispersa CLI', () => {
  ...
  it('reports generated type definitions in build output when types is configured', async () => {
    const tempDir = join(tmpdir(), `dispersa-cli-types-${Date.now()}`)
    await mkdir(tempDir, { recursive: true })
    ...
    const typesContent = await readFile(join(tempDir, 'dist', 'tokens.d.ts'), 'utf8')
    expect(typesContent).toContain('export type TokenName =')

    await rm(tempDir, { recursive: true, force: true })
  })

  it('does not report a types output line when types is omitted', async () => {
    const tempDir = join(tmpdir(), `dispersa-cli-notypes-${Date.now()}`)
    await mkdir(tempDir, { recursive: true })
