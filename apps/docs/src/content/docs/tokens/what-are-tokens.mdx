...
The [Design Tokens Community Group (DTCG)](https://designtokens.org/) maintains an open specification that defines how tokens are structured and referenced. The **2025.10** standard covers token format, types, references, and the resolver document for assembling token sets. Dispersa implements this specification so your tokens stay portable and standards-compliant.
