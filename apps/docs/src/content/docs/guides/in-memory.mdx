...
## Omit buildPath — Output Stays in Memory
...
Use `resolveTokens()` to resolve tokens for a single permutation without rendering:
...
```typescript
const allPermutations = await dispersa.resolveAllPermutations('./tokens.resolver.json')
