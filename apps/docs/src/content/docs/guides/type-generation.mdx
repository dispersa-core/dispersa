...
Type generation uses the resolved tokens, not the raw token files. Run it after resolution to capture all aliases and modifier values.
</Aside>

<LinkCard
  title="API Reference — Dispersa Class"
