...
<Aside type="note" title="enabled only gates build()">
  `lint.enabled` only gates the linting embedded in `build()`. The `dispersa lint` CLI command lints
  independently of `build()` and always runs, regardless of this flag.
</Aside>
