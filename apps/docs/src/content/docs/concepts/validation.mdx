...
Dispersa validates your token files and resolver documents against the official [DTCG 2025.10 JSON schemas](https://www.designtokens.org/TR/2025.10/) before any transforms or rendering run. Malformed tokens, invalid resolver structures, and spec violations are caught early — not buried in broken output.
...
Disabling validation can be useful during migration from a non-DTCG format, but it means Dispersa won't catch malformed tokens. Prefer `'warn'` over `'off'` when possible.
</Aside>
