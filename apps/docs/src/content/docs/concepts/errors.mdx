...
Available error classes: `DispersaError` (base), `TokenReferenceError`, `CircularReferenceError`, `ValidationError`, `FileOperationError`, `ConfigurationError`, `BasePermutationError`, `ModifierError`, `LintError`.

`LintError` is thrown by `lint()` and `build()` when lint errors are found and `failOnError` is `true` (the default). It carries `issues`, `errorCount`, and `warningCount`.
