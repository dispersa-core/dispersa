...
Builder functions create `OutputConfig` objects used in the `outputs` array of `BuildConfig`. Each builder returns a configured output ready to pass to `build()`.

Import from `dispersa` or `dispersa/outputs`:

```typescript
import { css, json, js, tailwind, ios, android } from 'dispersa'
// or
import { css, json, js, tailwind, ios, android } from 'dispersa/outputs'
```
...
<Aside type="tip" title="Presets">
  Presets control how permutations map to files. `bundle` = one file per output; `standalone` = one
  file per permutation; `modifier` (CSS only) = files grouped by modifier.
</Aside>
