...
| lintResult | `LintResult`                                         | Present when `lint.enabled` is `true`, on both successful and failed builds |
...
`'TOKEN_REFERENCE' | 'CIRCULAR_REFERENCE' | 'VALIDATION' | 'FILE_OPERATION' | 'CONFIGURATION' | 'BASE_PERMUTATION' | 'MODIFIER' | 'LINT' | 'UNKNOWN'`
