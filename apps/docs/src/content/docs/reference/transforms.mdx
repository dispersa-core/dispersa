...
All color transforms accept color tokens. Input: DTCG color object. Output: string in target format.
...
Transform token paths/names. Input: path string (e.g. `color.brand.primary`). Output: formatted name.
...
| durationToMs() | `durationToMs` | `{ value: 200, unit: 'ms' }` | `'200ms'` |
| durationToSeconds() | `durationToSeconds` | `{ value: 200, unit: 'ms' }` | `'0.2s'` |
