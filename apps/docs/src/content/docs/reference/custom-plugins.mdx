...
const a11yPlugin = await loader.load('@dispersa/lint-plugin-a11y')

// Load several plugins at once
const plugins = await loader.loadAll({
  a11y: '@dispersa/lint-plugin-a11y',
  myPlugin: './lint-plugins/my-design-system.ts',
})
```
...
Loaded plugins are cached by source. Call `loader.clearCache()` to force a reload.
