...
byPath('color.action')   // color.action.brand, color.action.danger, etc.
byPath('spacing.gap')    // spacing.gap.sm, spacing.gap.md, spacing.gap.lg, etc.
byPath('typography.heading') // typography.heading.lg, typography.heading.md, etc.
```
...
```typescript
byPath(/^color\.(action|text)/)       // color.action.* or color.text.*
byPath(/spacing\.gap\.(sm|md|lg)$/)  // Ends with spacing.gap.sm, .md, or .lg
byPath(/\.default$/)                 // Any path ending in .default
```
...
filters: [
  byType('color'),
  byPath('color.palette'),
  isBase(),
]
// Only base color tokens under color.palette.*
```
