...
| file | string or function | — | Output path |
| preset | 'bundle' \| 'standalone' | 'standalone' | Output preset |
| structure | 'flat' \| 'nested' | — | Token structure |
...
When `generateHelper` is true (bundle mode), a `getToken()` lookup function is added:
...
The helper allows looking up tokens by path string (e.g., `getToken('color.brand.primary')`).
