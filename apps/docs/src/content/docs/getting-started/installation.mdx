...
<Aside type="caution">
  A global install is convenient for quick one-off builds, but for team projects you should still add `dispersa` as a local dependency. This ensures everyone on the team uses the same version.
</Aside>
